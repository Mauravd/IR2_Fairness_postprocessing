...
from codes.src.measures.calculateOveralDTR import *
import os
import pandas as pd
import csv
import datetime
...
def main_1(dataSetName, fileNames, listNetRanking, queryNumbers, query_seq_file):
    results = []
    ...
    listNet_results.to_csv(os.path.join(result_path, 'ListNet_' + '.csv'))
    fair_results.to_csv(os.path.join(result_path, 'Fair_' + '.csv'))
...
def main():
    ...
    for dirpath, dirnames, files in os.walk(data_path):
        ...
        if 'fold' in dirpath:
            ...
            main_1(dataSetName, fileNames, listNetRanking, queryNumbers, query_seq_file)
...
def evaluateLearning(algoName, ranking, dataSetName, queryNumbers, listNet=False, k=10, query_seq=None):
    """
...
def scoreBasedEval(dataSetName, dataSetPath, k=10, protected=[], nonProtected=[], originalRanking=[],
                   listNet=False, query_rep=1):
    """
