...
class Learner:
    def __init__(self, PPG_mat, samples_cnt, objective_ins, sorted_docs, intra, sessions_cnt) -> None:
    ...
    def fit(self, epochs, lr, verbose=0):
        ...
        if self.verbose > 0:
            print(self.ref_permutation, 'intra:', self.intra)
...
#                 b = _PPG_sample(self.PPG)
        ...
        if self.sessions_cnt == 1:
            return self.ref_permutation[min_b]
        
        
        output = []
        for i in range(self.sessions_cnt):
            b = self.ref_permutation[_PPG_sample(self.PPG)] + (i * self.n)
            output.append(b)
